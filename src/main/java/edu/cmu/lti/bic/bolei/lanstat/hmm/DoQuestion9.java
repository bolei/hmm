...
public class DoQuestion9 {
	public static void main(String[] args) {
		...
		BufferedReader brIn = null, brInDecode = null;
		try {
			...
			brInDecode = new BufferedReader(new InputStreamReader(
					HMM.class.getResourceAsStream(config
							.getProperty("decodecorpus"))));
			String decodeStream = brInDecode.readLine();
			new ViterbiDecoder().decode(hmm, decodeStream);
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			if (brIn != null) {
