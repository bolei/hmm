...
public class DoQuestion8 {
	public static void main(String[] args) {
		...
		BufferedReader brIn = null, brInTest = null;
		try {
			...
			// test
			brInTest = new BufferedReader(new InputStreamReader(
					HMM.class.getResourceAsStream(config
							.getProperty("testcorpus"))));
			String testStream = brInTest.readLine();
			double avgll = new ForwardAlgorithmHMMEvaluator().evaluate(
					testStream, hmm);
			System.out.println("test result:" + avgll);
